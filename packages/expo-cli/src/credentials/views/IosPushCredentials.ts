...
export class CreateOrReusePushKey implements IView {
  ...
  async _createOrReuse(ctx: Context): Promise<IView | null> {
    if (ctx.nonInteractive) {
      throw new CommandError(
        'NON_INTERACTIVE',
        "Start the CLI without the '--non-interactive' flag to select an existing push key or create a new one."
      );
    }
...
async function selectPushCredFromList(
  ...
  if (ctx.nonInteractive) {
    throw new CommandError(
      'NON_INTERACTIVE',
      "Start the CLI without the '--non-interactive' flag to select a push notification credential."
    );
  }
  ...
  try {
    ...
    if (e.code === 'APPLE_PUSH_KEYS_TOO_MANY_GENERATED_ERROR') {
      ...
      if (ctx.nonInteractive) {
        throw new CommandError(
          'NON_INTERACTIVE',
          "Start the CLI without the '--non-interactive' revoke push notification keys."
        );
      }
