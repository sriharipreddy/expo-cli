...
export class CreateOrReuseDistributionCert implements IView {
  ...
  async _createOrReuse(ctx: Context): Promise<IView | null> {
    if (ctx.nonInteractive) {
      throw new CommandError(
        'NON_INTERACTIVE',
        "Start the CLI without the '--non-interactive' flag to select a certificate or create a new ones."
      );
    }
...
async function selectDistCertFromList(
  ...
  if (ctx.nonInteractive) {
    throw new CommandError(
      'NON_INTERACTIVE',
      "Start the CLI without the '--non-interactive' flag to select a certificate."
    );
  }
